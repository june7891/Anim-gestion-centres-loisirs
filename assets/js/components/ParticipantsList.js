...
const ParticipantsList = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    axios.get(`/api/participants?enterprise=${enterprise}`).then((response) => {
  ...
  const handleParticipantsFilter = (event) => {
    ...
    if (event.target.value === "alphabet") {
      ...
      axios
        .get(`/api/participants?enterprise=${enterprise}`)
        .then((response) => {
          setParticipants(
    ...
    if (event.target.value === "Maternelle") {
      axios
        .get(
          `/api/participants?enterprise=${enterprise}&schoolType.type=${event.target.value}`
        )
        .then((response) => {
          setParticipants(response.data["hydra:member"]);
          // console.log(data);
  ...
  const deleteParticipant = () => {
    axios.delete(`/api/participants/${participantId}`).then((response) => {
  ...
  return (
    <>
      {participants && participants.length === 0 ? (
        <h4>Vous n'avez aucun participant enregistré</h4>
      ) : (
      ...
      <div className="filter-bar">
        ...
        <div className="filter-icons">
          <input
            ...
            onClick={handleParticipantsFilter}
          />
          <input
            className="order-btn"
            type="image"
            src={alphabetReverseIcon}
      ...
      <table className="table mx-auto my-5">
        ...
        <tbody>
          {loading ? (
            ...
            participants
              .filter((participant) =>
                participant.lastname.toLowerCase().startsWith(filter)
              )
